...
# verificare la funzionalità senza sudo
  sudo wget $zipUrl -O /tmp/$zipName
else
# verificare la funzionalità senza sudo
  sudo curl -Lo /tmp/$zipName -O $zipUrl
fi

sudo unzip -o /tmp/$zipName -d /opt/wordpress

#json=$(printf "$template" "0" "WordPress installation succeeded")
json=$(printf "$template" "0" "{\\\"zip.url\\\":\\\"$zipUrl\\\"}")
echo "$json"
