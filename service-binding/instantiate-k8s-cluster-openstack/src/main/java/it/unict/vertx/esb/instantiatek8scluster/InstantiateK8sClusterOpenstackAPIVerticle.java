...
public class InstantiateK8sClusterOpenstackAPIVerticle extends AbstractVerticle implements InstantiateCluster {
	...
	@Override
	  public void start(Future<Void> future) throws Exception {
		...
		projectId = config().getString("project.id"); // 68f1e8f6d65c4f68b035d086f5a8edc4
...
//		adminPassword = config().getString("admin.password");
	...
	public void createCluster(RoutingContext routingContext) {
		...
		if (properties.get("platform").equals("swarm")) return;
...
//                .scopeToDomain(Identifier.byId(domainId))
		...
		case "CREATE_COMPLETE":
			...
			Address masterAddress = servers.get(0).getAddresses().getAddresses().get("private").stream()
