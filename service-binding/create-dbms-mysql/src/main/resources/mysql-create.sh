...
now=$(date)
echo "MySQL-DBMS-CREATE --> $now" >> ~/log.txt
...
sudo apt-get -y install --fix-missing mysql-server

rm -rf "${LOCK}"
echo "$NAME released apt lock"

#json=$(printf "$template" "0" "MySQL installation succeeded")
json=$(printf "$template" "0" "{\\\"db.root.pwd\\\":\\\"$db_root_password\\\"}")
echo "$json"
