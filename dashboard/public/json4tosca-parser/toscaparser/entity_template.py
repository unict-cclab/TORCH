...
class EntityTemplate(object):
    ...
    def artifacts(self):
        if self._artifacts is None:
            self._artifacts = self._create_artifacts()
    ...
    def _create_artifacts(self):
        ...
        if isinstance(self.type_definition, NodeType):
            type_artifacts = self.type_definition.get_value(self.ARTIFACTS,
                                                    self.entity_tpl)
        else:
            return None
        if type_artifacts:
            for artifact_type, value in type_artifacts.items():
                artifacts.append({ 'name': artifact_type, **value})
                # TODO: use the same approach of other entities
                # for op, op_def in value.items():
                #     iface = InterfacesDef(self.type_definition,
                #                           interfacetype=artifact_type,
                #                           node_template=self,
                #                           name=op,
                #                           value=op_def)
                #     artifacts.append(iface)
        return artifacts
